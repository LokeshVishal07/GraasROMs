...
from lib.auth import require_login, current_user, current_permissions, logout, role_label, cleanup_demo_accounts
...
ensure_seed()
cleanup_demo_accounts()  # deactivates the original demo logins once real accounts exist -- see lib/auth.py
...
PAGES = [
    ...
    ("WH Inspection Bulk Upload", "inspect"),
    ("Damaged Queue", "view_damaged_queue"),
    ("Return Not Received", "view_damaged_queue"),
    ("Refund Only", "view_damaged_queue"),
...
with st.sidebar:
    ...
    # A page (Raise Claim, a KPI drill-down, ...) sets nav_target + reruns to
    # jump the user somewhere else. index= on st.radio only seeds the very
    # first run of the widget's lifetime in this session -- once "nav_radio"
    # has a stored value (which it will, after the first page load), index=
    # is silently ignored on every later run. Writing directly into
    # session_state["nav_radio"] is the only way to actually move the
    # selection on a later rerun, so that's what makes nav_target work at all
    # past the very first navigation of the session.
    if "nav_target" in st.session_state:
        target = st.session_state.pop("nav_target")
        if target in available:
            st.session_state["nav_radio"] = target
    page = st.radio("Navigate", available, index=0, label_visibility="collapsed", key="nav_radio")
...
if page == "Dashboard":
    ...
    returns.render()
elif page == "WH Inspection Bulk Upload":
    from pages_app import bulk_upload
    bulk_upload.render()
elif page == "Damaged Queue":
    from pages_app import damaged
    damaged.render()
elif page == "Return Not Received":
    from pages_app import not_received
    not_received.render()
elif page == "Refund Only":
    from pages_app import refund_only
    refund_only.render()
