...
def build_template_bytes() -> bytes:
    ...
    ins.cell(row=note_row, column=2, value=(
        ...
        "Order ID matches more than one order and Return ID doesn't narrow it down; or comments/evidence "
        "link are missing for a Damaged or Not Received row. Inspection Status is no longer required on "
        "every row, and the same Order ID (or Order ID + Return ID) is allowed to repeat across multiple "
        "rows — each one is applied.")).alignment = Alignment(wrap_text=True)
    ...
    for i, row in df.iterrows():
        ...
        if row_errors:
            ...
            if needs_review:
                ...
                row_comments = f"{note} {comments}".strip() if comments else note
            valid_rows.append({
