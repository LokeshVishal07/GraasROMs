...
def load_orders_df(_nonce) -> pd.DataFrame:
    ...
    try:
        ...
        for r in rows:
            recs.append({
                ...
                "shipping_status": r.shipping_status, "order_dispatched_ts": r.order_dispatched_ts,
    ...
    df["order_dispatched_ts"] = pd.to_datetime(df["order_dispatched_ts"])
    return df
...
def load_tickets_df(_nonce) -> pd.DataFrame:
    ...
    try:
        ...
        recs = [{
            ...
            "internal_notes": t.internal_notes, "appeal_status": t.appeal_status,
    ...
    try:
        ...
        for e in evidence_rows:
            ...
            recs.append({
                ...
                "content_type": e.content_type, "size": e.size, "category": e.category,
...
def apply_filters(df, preset, custom_start, custom_end, marketplaces, channels, brands,
    ...
    if start and end:
        # Compare as Timestamps rather than out["order_date"].dt.date >= start:
        # when a datetime64 column is entirely NaT (e.g. no orders have an
        # order_date set), pandas' .dt.date accessor doesn't fall back to an
        # object dtype of date values as it normally would -- it stays
        # datetime64[ns], and comparing that directly to a plain date raises
        # TypeError: Invalid comparison between dtype=datetime64[ns] and date.
        # Comparing Timestamps to the datetime64 column directly sidesteps
        # that pandas quirk entirely and NaT rows just compare False, as
        # intended (they're correctly excluded from a date-range filter).
        start_ts = pd.Timestamp(start)
        end_ts = pd.Timestamp(end) + pd.Timedelta(days=1)
        mask = (out["order_date"] >= start_ts) & (out["order_date"] < end_ts)
        out = out[mask]
