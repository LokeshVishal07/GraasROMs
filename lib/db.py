        ...
        with _engine_lock:
            ...
            if _engine is None:
                ...
                try:
                    ...
                    # Benign race: Streamlit Cloud can spin up more than one
                    # script run against a brand-new (empty) database at
                    # almost the same instant. Both see "table does not
                    # exist yet" and both issue CREATE TABLE; one wins, the
                    # other hits "table already exists". The tables are
                    # there either way, so this specific error is safe to
                    # ignore rather than crash the app.
