...
def save_uploaded_file(order_key: str, uploaded_file, user_name: str, when=None) -> None:
    session = get_session()
    try:
...
def save_link(order_key: str, url: str, user_name: str, when=None) -> None:
    session = get_session()
    try:
        session.add(Evidence(
...
def list_evidence(order_key: str):
    session = get_session()
    try:
        return session.query(Evidence).filter_by(order_key=order_key).order_by(
...
def render_evidence_gallery(order_key: str, key_prefix: str = "") -> list:
    """Renders every evidence row for an order and returns the list, so callers
    (e.g. the claims page) can also offer a 'download all as ZIP' button."""
    items = list_evidence(order_key)
    if not items:
