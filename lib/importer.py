...
def _upsert_return_orders(session, mappings, chunk_size=1000):
    ...
    known_cols = set(all_cols) | {"key"}
    update_cols = [c for c in all_cols if c not in _WAREHOUSE_OWNED_COLS]

    # Defensive: only pass along keys the currently-loaded ORM model actually
    # has a column for. Without this, a mapping dict carrying a field the
    # model doesn't (yet) recognize -- e.g. a partial deploy where importer.py
    # rolled out slightly ahead of a db.py schema change -- crashes the whole
    # import with a SQLAlchemy CompileError ("Unconsumed column names: ...")
    # instead of just silently dropping the one extra field.
    safe_mappings = [{k: v for k, v in m.items() if k in known_cols} for m in mappings]

    for i in range(0, len(safe_mappings), chunk_size):
        batch = safe_mappings[i:i + chunk_size]
...
def classify_trigger(order_status, shipping_status, cancel_reason, channel):
    ...
    if order_status == "CANCELLED":
        ...
        # shipping status or a recognized delivery-fail reason never left
        # the warehouse -- it gets its own bucket (rather than being lumped
        # into "Returned" or dropped into "Other") so Ops and the dashboard
        # can tell "shipped, then cancelled" apart from "cancelled before it
        # ever shipped." This only affects orders already captured by the
        # extraction filter, not new ones.
        return "cancelled_not_shipped", "Cancelled — Not Shipped"
