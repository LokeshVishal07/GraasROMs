...
from lib.db import get_session, User, RolePermission, _engine_lock
...
def ensure_seed():
    # Serialize with the same lock used for engine/table setup: on Streamlit
    # Cloud more than one script run can hit a brand-new (empty) database at
    # almost the same instant, and without this lock two runs can both see
    # "0 users" and both try to insert the same default accounts, causing a
    # UNIQUE constraint / IntegrityError crash. The lock makes the second
    # run wait until the first one has already committed the seed data.
    with _engine_lock:
        session = get_session()
        try:
            ...
            # Benign race backstop: another run won the seeding race between
            # our count() check and our commit. The data is already there,
            # so just roll back our half-finished insert and move on.
            session.rollback()
        finally:
            session.close()
