...
from lib.db import get_session, User, RolePermission, ReturnOrder, Evidence
...
from lib.utils import log_activity
...
def _render_data_import():
    ...
    st.divider()
    _render_reset_inspection()


def _render_reset_inspection():
    st.markdown("#### Reset Inspection Data")
    st.caption("Clear warehouse inspection results back to Pending — handy for wiping out test data "
               "before going live. Only orders that have already been inspected show up here; resetting "
               "clears the inspection status, comments, damage reason, warehouse, tracking number, and "
               "who/when inspected. Orders remain in the system either way — only the inspection is reset.")

    session = get_session()
    try:
        orders = session.query(ReturnOrder).filter(ReturnOrder.inspection_status != "pending").order_by(
            ReturnOrder.inspected_at.desc().nullslast()).all()
        if not orders:
            st.info("Nothing to reset — every order is already Pending.")
            return
        rows = []
        for o in orders:
            ev_count = session.query(Evidence).filter_by(order_key=o.key).count()
            rows.append({
                "key": o.key, "Order ID": o.order_id, "Brand": o.brand, "Marketplace": o.marketplace,
                "Status": o.inspection_status, "Inspected By": o.inspected_by or "—",
                "Inspected At": o.inspected_at.strftime("%d %b %Y, %H:%M") if o.inspected_at else "—",
                "Evidence": ev_count,
            })
    finally:
        session.close()
    ...
    edited = st.data_editor(
        editable, hide_index=True, width="stretch", height=min(360, 60 + 35 * len(rows)), key="reset_editor",
        disabled=["Order ID", "Brand", "Marketplace", "Status", "Inspected By", "Inspected At", "Evidence"],
        column_config={"Select": st.column_config.CheckboxColumn(required=True)},
        column_order=["Select", "Order ID", "Brand", "Marketplace", "Status", "Inspected By",
                      "Inspected At", "Evidence"],
    )
    ...
    also_delete_evidence = st.checkbox(
        "Also delete uploaded evidence (files & links) for these orders", value=True, key="reset_del_evidence")
    confirm = st.checkbox(
        f"Yes, reset {len(selected_keys)} order(s) back to Pending"
        + (" and delete their evidence" if also_delete_evidence else ""),
        key="reset_confirm",
    )
    if st.button(f"Reset {len(selected_keys)} order(s)", type="primary", disabled=not confirm):
        session = get_session()
        try:
            for key in selected_keys:
                ...
                order.inspection_comment = None
                order.damage_reason = None
                order.inspected_by = None
                order.inspected_at = None
                order.warehouse = None
                order.tracking_number = None
                if also_delete_evidence:
                    session.query(Evidence).filter_by(order_key=key).delete()
                session.commit()
                log_activity(order_key=key, action="Inspection reset",
                             comment="Reset to Pending" + (" (evidence deleted)" if also_delete_evidence else ""))
        finally:
            session.close()
        bump_cache_nonce()
        st.success(f"Reset {len(selected_keys)} order(s) to Pending.")
        st.rerun()
