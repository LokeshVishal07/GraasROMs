...
from lib.bulk_import import (
    ...
    build_error_report_bytes, apply_bulk_upload, STATUS_MAP,
)


def render():
    st.subheader("Warehouse Bulk Upload")
    ...
    label_by_code = {v: k.title() for k, v in STATUS_MAP.items()}
    preview = pd.DataFrame([{
        "Row": r["row"], "Order ID": r["order_id"], "Status": label_by_code.get(r["status_code"], r["status_code"]),
    ...
    if st.button(f"Apply {len(valid_rows)} valid row(s)", type="primary"):
        ...
        bump_cache_nonce()
        st.success(f"Applied {result['applied']} inspection update(s).")
        st.rerun()
