...
def render():
    ...
    # Same shared filter bar as Dashboard/Returns -- lets Ops narrow this queue
    # by Brand, Marketplace, and MP Status (return/refund, delivery failed,
    # cancelled after shipped, etc.) instead of scrolling the whole backlog.
    f = filter_bar(df, "damagedq")
    ...
    queue = fdf[fdf["inspection_status"].isin(["damaged", "not_received"])].sort_values(
    ...
    if queue.empty:
        st.info("Nothing in the damaged / not received queue matches the current filters.")
        return
