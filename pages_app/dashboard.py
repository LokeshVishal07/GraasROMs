...
def _goto_returns(status=None, trigger=None):
    """Used as a KPI tile's on_click: pre-loads the Returns page's own filter
    widgets (by key) before navigating there, so the tile behaves as a
    drill-down into exactly the orders it's counting."""
    if status is not None:
        st.session_state["returns_status"] = [status]
    if trigger is not None:
        st.session_state["returns_trigger"] = [trigger]
    st.session_state["nav_target"] = "Returns"
...
def render():
    ...
    kpi_row([
        ("Total Returns", len(fdf), "#4f46e5", lambda: _goto_returns()),
        ("Pending Inspection", int((fdf["inspection_status"] == "pending").sum()) if not fdf.empty else 0,
         "#d97706", lambda: _goto_returns(status="pending")),
        ("Good Condition", int((fdf["inspection_status"] == "good").sum()) if not fdf.empty else 0,
         "#16a34a", lambda: _goto_returns(status="good")),
        ("Damaged", int((fdf["inspection_status"] == "damaged").sum()) if not fdf.empty else 0,
         "#e11d48", lambda: _goto_returns(status="damaged")),
    ])
    st.write("")
    kpi_row([
        ("Not Received", int((fdf["inspection_status"] == "not_received").sum()) if not fdf.empty else 0,
         "#0284c7", lambda: _goto_returns(status="not_received")),
        ("Refund Only", int((fdf["inspection_status"] == "refund_only").sum()) if not fdf.empty else 0,
         "#7c3aed", lambda: _goto_returns(status="refund_only")),
        ("Cancelled After Shipped", int((fdf["trigger_code"] == "cancelled_after_shipped").sum()) if not fdf.empty else 0,
         "#b45309", lambda: _goto_returns(trigger="cancelled_after_shipped")),
        ("Cancelled — Not Shipped", int((fdf["trigger_code"] == "cancelled_not_shipped").sum()) if not fdf.empty else 0,
         "#9333ea", lambda: _goto_returns(trigger="cancelled_not_shipped")),
        ("Delivery Failed", int((fdf["trigger_code"] == "delivery_failed").sum()) if not fdf.empty else 0,
         "#e11d48", lambda: _goto_returns(trigger="delivery_failed")),
        ("Open Marketplace Cases", int(scoped_tickets["status"].isin(["Open", "Follow-up", "Waiting for Marketplace"]).sum()) if not scoped_tickets.empty else 0,
         "#0284c7", _goto_claims),
    ])
