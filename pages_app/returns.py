...
def _render_bulk_update(show: pd.DataFrame):
    ...
    with st.form("bulk_update_form"):
        ...
        evidence_files = st.file_uploader(
            "Shared evidence — mandatory for Damaged / Not Received (applied to every selected order)",
            accept_multiple_files=True, type=["png", "jpg", "jpeg", "gif", "mp4", "mov", "pdf"],
        )
    ...
    if status in ("damaged", "not_received"):
        ...
        if not evidence_files:
            st.error("Evidence upload is mandatory for Damaged / Not Received.")
            return
    ...
    try:
        ...
        for key in selected_keys:
            ...
            for f in (evidence_files or []):
                session.add(Evidence(order_key=key, filename=f.name, content_type=f.type,
...
def _render_detail(row, perms):
    ...
    if row["trigger_code"] == "cancelled_after_shipped":
        st.warning(f"This order shipped, then was cancelled ({row['cancelled_by'] or 'unknown'} — "
                   f"{row['cancel_reason'] or 'no reason given'}). The marketplace already flags the parcel "
                   f"as returned to the warehouse, so it's tracked here even though its marketplace status "
                   f"still reads \"Cancelled\".")
    ...
    with st.form(f"inspect_{row['key']}"):
        ...
        evidence_files = st.file_uploader(
            "Evidence — mandatory for Damaged / Not Received (images, video, PDF)",
            accept_multiple_files=True, type=["png", "jpg", "jpeg", "gif", "mp4", "mov", "pdf"],
        )
    ...
    try:
        ...
        if status in ("damaged", "not_received"):
            if not comment.strip():
                ...
                return
            if existing_evidence_count == 0 and not new_files:
                st.error("Evidence upload is mandatory for Damaged / Not Received.")
                return
        ...
        order.inspection_comment = comment.strip()
        order.inspected_by = user["name"]
        order.inspected_at = dt.datetime.utcnow()
        for f in new_files:
            session.add(Evidence(order_key=row["key"], filename=f.name, content_type=f.type,
                                  size=f.size, data=f.getvalue(), uploaded_by=user["name"],
