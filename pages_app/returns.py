...
DISPLAY_COLS = {
    ...
    "inspection_status": "Inspection", "sla_display": "Inspection SLA (48h)", "warehouse": "Warehouse",
    "tracking_number": "Tracking #",
}

INSPECTION_OPTIONS = ["pending", "good", "damaged", "not_received", "refund_only",
                       "request_cancelled", "lost_scrap"]
INSPECTION_LABELS = {"pending": "— Select —", "good": "Received in Good Condition",
                      "damaged": "Damaged", "not_received": "Not Received",
                      "refund_only": "Refund Only (buyer refunded, no item back)",
                      "request_cancelled": "Request Cancelled (buyer withdrew the return)",
                      "lost_scrap": "Lost & Scrap (lost in transit / written off)"}

# Comment explaining the outcome is required for anything other than a clean
# Good receipt. Evidence (photo/video/link) is only required for Damaged /
# Not Received, since a Refund Only case has no physical item to photograph --
# it's usually backed by a WH WhatsApp confirmation or marketplace screenshot
# instead, which can still be attached but isn't mandatory. Request Cancelled
# and Lost & Scrap need no comment/evidence either -- there's nothing for the
# warehouse to inspect or document in either case.
COMMENT_REQUIRED = {"damaged", "not_received", "refund_only"}
EVIDENCE_REQUIRED = {"damaged", "not_received"}
...
def _sla_cell_style(val: str) -> str:
    """Per-cell CSS for the Inspection SLA column: green while there's time
    left, red once it's overdue, plain for N/A ("—")."""
    if isinstance(val, str):
        if val.startswith("Overdue"):
            return "color: #dc2626; font-weight: 600;"
        if val.endswith("left"):
...
def render():
    ...
    show["sla_display"] = show.apply(
        lambda r: sla_text(r["return_date"], r["inspection_status"]), axis=1)
    display = show[list(DISPLAY_COLS.keys())].rename(columns=DISPLAY_COLS)
    styled = display.style.map(_sla_cell_style, subset=["Inspection SLA (48h)"])
...
def _render_detail(row, perms):
    ...
    sla_badge = sla_badge_html(row["return_date"], row["inspection_status"])
    st.markdown(
    ...
    if row["trigger_code"] == "cancelled_not_shipped":
        st.info(f"Cancelled within the warehouse — not shipped "
                 f"({row['cancelled_by'] or 'unknown'} — {row['cancel_reason'] or 'no reason given'}). "
                 f"There's no parcel in transit; it's tracked here only so Ops can follow up on the refund.")
    ...
    with st.form(f"inspect_{row['key']}"):
        ...
        comment = st.text_area("Comments (required for Damaged / Not Received / Refund Only)",
                                value=row["inspection_comment"] or "")
        damage_reason = st.text_input("Reason (if applicable)", value=row.get("damage_reason") or "")
        st.caption("Evidence — mandatory for Damaged / Not Received, optional for Refund Only. "
                   "Provide a file upload, a cloud storage link, or both.")
        evidence_files = st.file_uploader(
    ...
    try:
        ...
        if status in COMMENT_REQUIRED and not comment.strip():
            st.error("Comments are required for Damaged / Not Received / Refund Only.")
            return
        if (status in EVIDENCE_REQUIRED and existing_evidence_count == 0
                and not new_files and not evidence_link.strip()):
            st.error("Evidence is mandatory for Damaged / Not Received — upload a file or provide a link.")
            return
